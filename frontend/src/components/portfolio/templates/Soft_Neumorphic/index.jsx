...
const neu = {
  raised:
    "bg-[#e0e5ec] shadow-[6px_6px_14px_#b8bec7,-6px_-6px_14px_#ffffff] rounded-2xl",
  raisedSm:
    "bg-[#e0e5ec] shadow-[4px_4px_10px_#b8bec7,-4px_-4px_10px_#ffffff] rounded-xl",
  inset:
    "bg-[#e0e5ec] shadow-[inset_4px_4px_10px_#b8bec7,inset_-4px_-4px_10px_#ffffff] rounded-xl",
  insetSm:
    "bg-[#e0e5ec] shadow-[inset_3px_3px_7px_#b8bec7,inset_-3px_-3px_7px_#ffffff] rounded-lg",
  circle:
    "bg-[#e0e5ec] shadow-[6px_6px_14px_#b8bec7,-6px_-6px_14px_#ffffff] rounded-full",
};
...
const SectionHeading = ({ icon: Icon, title, subtitle }) => (
  <motion.div className="text-center mb-14" {...fadeUp()}>
    ...
    {subtitle && (
      <h2 className="text-3xl md:text-4xl font-bold text-gray-600 mt-3 font-serif">
        {subtitle}
      </h2>
    )}
  </motion.div>
  ...
  return (
    <section className="min-h-screen bg-[#e0e5ec] flex flex-col items-center justify-center px-6 py-20 relative overflow-hidden">
      ...
      <div className="max-w-5xl w-full grid md:grid-cols-2 gap-12 items-center z-10">
        ...
        <div>
          <motion.div
            className={`inline-flex items-center gap-2 px-4 py-2 ${neu.raisedSm} mb-6`}
            {...fadeUp(0)}
          >
          ...
          <motion.h1
            className="text-5xl md:text-6xl font-bold text-gray-600 font-serif leading-tight mb-4"
            {...fadeUp(0.1)}
          >
            {personal.name}
          </motion.h1>
          ...
          <motion.p
            className="text-lg text-indigo-400 font-medium mb-6"
            {...fadeUp(0.2)}
          >
            {personal.title}
          </motion.p>
          ...
          <motion.div className="flex flex-wrap gap-4" {...fadeUp(0.5)}>
            <a
              href="#projects"
              className="px-6 py-3 rounded-xl bg-indigo-400 text-white font-semibold shadow-[4px_4px_12px_#9fa8da,-2px_-2px_8px_#ffffff] hover:shadow-[2px_2px_6px_#9fa8da] transition-all duration-200"
            >
              View Projects
            </a>
            <a
              href="#contact"
              className={`px-6 py-3 ${neu.raisedSm} text-gray-500 font-semibold hover:text-indigo-400 transition-colors duration-200`}
            >
              Contact Me
            </a>
          </motion.div>
...
const About = () => {
  const { personal } = data;
  return (
    <section id="about" className="bg-[#e0e5ec] py-24 px-6">
          ...
          <motion.div {...fadeUp(0.1)}>
            <div className={`${neu.raised} p-6 flex flex-col items-center gap-5`}>
              <img
                src={personal.avatar}
                alt={personal.name}
                className="w-36 h-36 rounded-full object-cover shadow-[4px_4px_12px_#b8bec7,-4px_-4px_12px_#ffffff]"
              />
          ...
          <motion.div {...fadeUp(0.2)} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                { icon: Star, label: "Open Source Lover" },
...
const Skills = () => {
  ...
  return (
    <section id="skills" className="bg-[#e0e5ec] py-24 px-6">
      <div className="max-w-5xl mx-auto">
          ...
          {categories.map((cat, ci) => (
            <motion.div key={cat} {...fadeUp(ci * 0.1)}>
              <div className={`${neu.raised} p-6`}>
                <h3 className="text-sm font-semibold tracking-widest uppercase text-indigo-400 mb-6">
                  {cat}
                </h3>
...
const Projects = () => {
  const { projects } = data;
  return (
    <section id="projects" className="bg-[#e0e5ec] py-24 px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, i) => (
            <motion.div
              key={project.title}
              {...fadeUp(i * 0.08)}
              className={`${neu.raised} overflow-hidden flex flex-col group`}
            >
              {/* Image */}
              ...
              <div className="p-5 flex flex-col flex-1 gap-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.techStack.map((tech) => (
                    <span
                      key={tech}
                      className={`text-xs px-3 py-1 ${neu.insetSm} text-indigo-400 font-medium`}
                    >
                      {tech}
                    </span>
                  ))}
...
const Experience = () => {
  ...
  return (
    <section id="experience" className="bg-[#e0e5ec] py-24 px-6">
      <div className="max-w-3xl mx-auto">
        <SectionHeading icon={Briefcase} title="Experience" subtitle="Where I've Worked" />
        <div className="relative">
          {/* Vertical line */}
          <div className="absolute left-6 top-0 bottom-0 w-0.5 bg-gradient-to-b from-indigo-300 to-transparent" />

          <div className="space-y-8 pl-16">
            {experience.map((exp, i) => (
              <motion.div key={exp.company} {...fadeUp(i * 0.1)}>
                {/* Dot */}
                <div className={`absolute left-3.5 w-5 h-5 ${neu.circle} border-2 border-indigo-300`}
                  style={{ top: `${i * 140 + 8}px` }}
                />
                <div className={`${neu.raised} p-6`}>
...
const Testimonials = () => {
  ...
  return (
    <section id="testimonials" className="bg-[#e0e5ec] py-24 px-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 gap-8">
          {testimonials.map((t, i) => (
            <motion.div
              key={t.name}
              {...fadeUp(i * 0.1)}
              className={`${neu.raised} p-7 flex flex-col gap-5`}
            >
              {/* Quote mark */}
              <div className={`w-10 h-10 flex items-center justify-center ${neu.inset} text-indigo-300 text-2xl font-serif`}>
                "
              </div>
              ...
              <div className="flex items-center gap-4">
                <img
                  src={t.avatar}
                  alt={t.name}
                  className={`w-12 h-12 rounded-full object-cover shadow-[3px_3px_8px_#b8bec7,-3px_-3px_8px_#ffffff]`}
                />
                <div>
...
const Contact = () => {
  ...
  return (
    <section id="contact" className="bg-[#e0e5ec] py-24 px-6">
      <div className="max-w-2xl mx-auto">
        ...
        <motion.div {...fadeUp(0.1)} className={`${neu.raised} p-8 space-y-6`}>
          ...
          <div className="space-y-4">
            ...
            />
            <button className="w-full py-4 rounded-xl bg-indigo-400 text-white font-semibold shadow-[4px_4px_12px_#9fa8da,-2px_-2px_8px_#ffffff] hover:shadow-[2px_2px_6px_#9fa8da] transition-all duration-200 flex items-center justify-center gap-2">
